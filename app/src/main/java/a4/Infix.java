...
/**
 * This class provides methods for converting infix mathematical expressions to
 * postfix notation and evaluating the result using a stack-based algorithm.
 */
public class Infix {

    /**
     * Converts an infix expression (given as a deque of tokens) into postfix
     * notation. The expression can contain operators, operands, and
     * parentheses.
     *
     * @param tokens An ArrayDeque of Objects representing the infix expression,
     * where each element is either a Double (operand) or a Character
     * (operator).
     * @return Double representing the result of the postfix expression
     * evaluation.
     * @throws IllegalArgumentException If there are mismatched parentheses or
     * an invalid token in the input.
     */
    ...
    /**
     * Returns the precedence of the given operator. Precedence determines the
     * order of operations in an expression.
     *
     * @param op The operator whose precedence is to be evaluated.
     * @return An integer value representing the precedence level of the
     * operator. Higher values = higher precedence
     */
     ...
     * @return true if the operator is right-associative, otherwise, returns
     * false
     */
    ...
    /**
     * Determines whether the operator at the top of the stack should be popped
     * in order to respect operator precedence and associativity.
     *
     * @param stackOp The operator at the top of the stack
     * @param queueOp The operator to be added to the output queue
     * @return true if the operator on the stack should be pushed, otherwise,
     * false
     */
